...
lib.callback.register('bayview_drugs:weed:getitem', function(src, type, entity)
    local src = source
...
RegisterServerEvent("bayview_drugs:weed:giveitems")
AddEventHandler("bayview_drugs:weed:giveitems", function(type, entity)
    local src = source
        ...
        if dist <= 100 then
            ...
            if number == #Config.Weed.Pickup.RequiredItems then
                ...
                for k, v in pairs(Config.Weed.Pickup.AddItems) do
                    ...
                    AddItem(v.item, random, src)
                    Logs(src, "has picked up x"..random.." weed bud." )
                end
