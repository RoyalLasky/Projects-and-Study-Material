...
CreateThread(function()
    if GetResourceState(GetCurrentResourceName()) == 'started' then
        print('bayview_drugs STARTED ON VERSION: ' .. ver)
    end
end)
    ...
    if message ~= nil then
        if Config.Logs.enabled then
            ...
            if Config.Logs.type == "ox_lib" then
                lib.logger(source, "Drugs", GetPlayerName(source).." ".. message)
...
function RemoveItem(name, count, source)
    ...
    elseif Config.Framework == "qbcore" then
        ...
        lib.logger("Drugs", "Drugs - Inventory Remove", xPlayer.PlayerData.charinfo.firstname.. " " .. xPlayer.PlayerData.charinfo.lastname .. " had items removed: ".. count .. "of " .. name ..".")
    elseif Config.Framework == "standalone" then
        -- ADD YOUR FRAMEWORK
    end
end

lib.callback.register('bayview_drugs:getpolice', function(source)
    local policeCount = 0
    if Config.Framework == "ESX" then
...
if Config.Clothing == "qb-clothing" then
    RegisterServerEvent("bayview_drugs:loadPlayerSkin")
    AddEventHandler('bayview_drugs:loadPlayerSkin', function()
        local src = source
        ...
        if result[1] ~= nil then
            TriggerClientEvent("bayview_drugs:loadSkin", src, false, result[1].model, result[1].skin)
        else
            TriggerClientEvent("bayview_drugs:loadSkin", src, true)
        end
    end)
end

RegisterServerEvent("bayview_drugs:deleteprop", function(ent)
    local ent = NetworkGetEntityFromNetworkId(ent)
    DeleteEntity(ent)
end)


lib.callback.register('bayview_drugs:getitembyname', function(source, item)
    local src = source
            ...
            RegisterUsable(k .. i, function(source)
                ...
                if v.Remove then
                    if GetItem(v.RemoveItem .. i, v.RemoveItemCount, source) then
                        ...
                        TriggerClientEvent('bayview_drugs:consumables', source, v.ProgressBar, v.animation, v.duration,
                            v.effect, v.add)
                    end
                else
                    TriggerClientEvent('bayview_drugs:consumables', source, v.ProgressBar, v.animation, v.duration, v.effect, v.add)
                end
            end)
        end
    else
...
RegisterUsable('weed_wrap', function(source)
    TriggerClientEvent('bayview_drugs:consumables:menu', source, "weed_wrap")
end)

RegisterUsable('syringe', function(source)
    TriggerClientEvent('bayview_drugs:consumables:menu', source, "syringe")
end)

RegisterUsable('weed_papers', function(source)
    TriggerClientEvent('bayview_drugs:consumables:menu', source, "weed_papers")
end)

RegisterUsable('xanaxpack', function(source)
    TriggerClientEvent('bayview_drugs:consumables:progress', source, "xanax_pack")
end)

RegisterUsable('xanaxplate', function(source)
    TriggerClientEvent('bayview_drugs:consumables:progress', source, "xanax_plate")
end)

RegisterUsable('coke_figure', function(source)
    TriggerClientEvent('bayview_drugs:coke:menus', source, "FullFigure")
end)

RegisterUsable('coke_figureempty', function(source)
    TriggerClientEvent('bayview_drugs:coke:menus', source, "EmptyFigure")
end)

RegisterUsable('coke_figurebroken', function(source)
    TriggerClientEvent('bayview_drugs:coke:menus', source, "BrokenFigure")
end)

lib.callback.register('bayview_drugs:consumables:getitem', function(source, type)
...
RegisterServerEvent("bayview_drugs:consumables:giveitems")
AddEventHandler("bayview_drugs:consumables:giveitems", function(type)
    if type == "weed_wrap" then
        if GetItem("weed_package", 1, source) and GetItem("weed_wrap", 1, source) then
