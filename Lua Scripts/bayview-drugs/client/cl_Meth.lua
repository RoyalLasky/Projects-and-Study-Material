    ...
    onEnter = function ()
        if Config.InteractionType == "target" then
            ...
            target:AddCircleZone("MethElectricMenu", Config.Meth.Electricity.coords,
                ...
                { options = {
                    { event = "bayview_drugs:meth:progress", icon = "fas fa-car-battery", label = locale("turnonelt"),
                        menu = "ElectricON" },
                    { event = "bayview_drugs:meth:progress", icon = "fas fa-car-battery", label = locale("turnoffelt"),
                        menu = "ElectricOFF" }
                },
                    distance = 2.5
                }
            ...
            --    { options = {
            --        { event = "bayview_drugs:meth:progress", icon = "fas fa-door-open", label = locale("leave"),
            --            menu = "MethDoor2" }
            --    },
            --        distance = 2.5
            --    }
            ...
            target:AddCircleZone("MethPourMenu", Config.Meth.Pouring.coords, Config.Meth.Pouring.radius,
                ...
                { options = {
                    { event = "bayview_drugs:meth:menus", icon = "fas fa-fire-burner", label = locale("MethCook"),
                        menu = "MethPour",
            ...
            target:AddCircleZone("MethHeatMenu", Config.Meth.Heat.coords, Config.Meth.Heat.radius,
                ...
                { options = {
                    { event = "bayview_drugs:meth:progress", icon = "fas fa-fire", label = locale("MethHeat"), menu = "Heat",
                        canInteract = function()
                            if Electricity then
                                return true
                            end
            ...
            target:AddCircleZone("MethCompleteMenu", Config.Meth.Complete.coords, Config.Meth.Complete.radius,
                ...
                { options = {
                    { event = "bayview_drugs:meth:progress", icon = "fas fa-box-open", label = locale("MethFinish"),
                        menu = "Complete",
                        canInteract = function()
                            if Electricity then
                                return true
            ...
            target:AddCircleZone("MethPackageMenu", Config.Meth.Package.coords, Config.Meth.Package.radius,
                ...
                { options = {
                    { event = "bayview_drugs:meth:menus", icon = "fas fa-box", label = locale("MethPack"),
                        menu = "MethPackage",
                        canInteract = function()
                            if Electricity then
                                return true
            ...
            target:AddCircleZone("MethBreakMenu", Config.Meth.Break.coords, Config.Meth.Break.radius,
                ...
                { options = {
                    { event = "bayview_drugs:meth:menus", icon = "fas fa-hammer", label = locale("MethBreak"),
                        menu = "MethBreak",
                        canInteract = function()
                            if Electricity then
                                return true
            ...
            methlabzones[#methlabzones+1] = lib.zones.sphere({
                ...
                inside = function(self)
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent("bayview_drugs:meth:progress", Electricity and {menu = "ElectricOFF"} or {menu = "ElectricON"})
                    end
            ...
            methlabzones[#methlabzones+1] = lib.zones.sphere({
                ...
                inside = function(self)
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent("bayview_drugs:meth:progress", {menu = "MethDoor2"})
                    end
            ...
            methlabzones[#methlabzones+1] = lib.zones.sphere({
                ...
                inside = function(self)
                    if Electricity then
                        if IsControlJustReleased(0, 38) then
                            TriggerEvent("bayview_drugs:meth:menus", {menu = "MethPour"})
                        end
            ...
            methlabzones[#methlabzones+1] = lib.zones.sphere({
                ...
                inside = function(self)
                    if Electricity then
                        if IsControlJustReleased(0, 38) then
                            TriggerEvent("bayview_drugs:meth:progress", {menu = "Heat"})
                        end
            ...
            methlabzones[#methlabzones+1] = lib.zones.sphere({
                ...
                inside = function(self)
                    if Electricity then
                        if IsControlJustReleased(0, 38) then
                            TriggerEvent("bayview_drugs:meth:progress", {menu = "Complete"})
                        end
            ...
            methlabzones[#methlabzones+1] = lib.zones.sphere({
                ...
                inside = function(self)
                    if Electricity then
                        if IsControlJustReleased(0, 38) then
                            TriggerEvent("bayview_drugs:meth:menus", {menu = "MethPackage"})
                        end
            ...
            methlabzones[#methlabzones+1] = lib.zones.sphere({
                ...
                inside = function(self)
                    if Electricity then
                        if IsControlJustReleased(0, 38) then
                            TriggerEvent("bayview_drugs:meth:progress", {menu = "MethBreak"})
                        end
...
RegisterNetEvent("bayview_drugs:meth:menus", function(data)
                ...
                if Config.Context == "qbcore" then
                    exports['qb-menu']:openMenu({
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'bayview_drugs:meth:progress',
                                args = { menu = data.menu }
                            }
                        }
                    })
...
-- PROGRESSY --
RegisterNetEvent("bayview_drugs:meth:progress")
AddEventHandler("bayview_drugs:meth:progress", function(data)
    --if data.menu == "MethDoor1" then
    --    if data.Card then
    --        lib.callback('bayview_drugs:getitembyname', false, function(value)
    --            if not value then
    ...
    --        exports.ox_target:disableTargeting(false)
    --end
    if data.menu == "MethGet" then
        lib.callback('bayview_drugs:meth:getitem', false, function(value)
            if value then
                ...
                TSE("bayview_drugs:meth:giveitems", data.menu)
            else
        ...
        lib.callback('bayview_drugs:meth:getitem', false, function(value)
            if value then
                if not IsProgressBarActive() then
                    ...
                    local netScene = NetworkCreateSynchronisedScene(x + 0.81, y - 5.2, z - 0.4, targetRotation, 2, false,
                        false, 1148846080, 0, 1.3)
                    local netScene2 = NetworkCreateSynchronisedScene(x + 0.81, y - 5.2, z - 0.4, targetRotation, 2, false
                        , false, 1148846080, 0, 1.3)
        ...
        lib.callback('bayview_drugs:meth:getitem', false, function(value)
            if value then
                if not IsProgressBarActive() then
                    ...
                    local netScene = NetworkCreateSynchronisedScene(x - 0.55, y + 5.69, z - 1.0, targetRotation, 2, false,
                        false, 1148846080, 0, 1.3)
                    local netScene2 = NetworkCreateSynchronisedScene(x - 0.55, y + 5.69, z - 1.0, targetRotation, 2, false
                        , false, 1148846080, 0, 1.3)
                    local netScene3 = NetworkCreateSynchronisedScene(x - 0.55, y + 5.69, z - 1.0, targetRotation, 2, false
                        , false, 1148846080, 0, 1.3)
...
SetTimeout(2000, function()
    ...
    if Config.Meth.Enterlab.NeedItem then
        if Config.InteractionType ~= "target" then
            lib.zones.sphere({
                ...
                inside = function(self)
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent("bayview_drugs:meth:progress", {menu = "MethDoor1", Card = Config.Meth.Enterlab.ItemName})
                    end
            ...
            target:AddCircleZone("MethTeleportMenu1", Config.Meth.Enterlab.coords, Config.Meth.Enterlab.radius,
                ...
                { options = {
                    { event = "bayview_drugs:meth:progress", icon = "fas fa-door-open", label = locale("enter"),
        ...
        if Config.InteractionType ~= "target" then
            lib.zones.sphere({
                ...
                inside = function(self)
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent("bayview_drugs:meth:progress", {menu = "MethDoor1"})
                    end
    ...
    if Config.InteractionType ~= "target" then
        lib.zones.sphere({
            ...
            inside = function(self)
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("bayview_drugs:meth:progress", {menu = "MethGet"})
                end
        ...
        target:AddCircleZone("MethGet", Config.Meth.GetSacid.coords, Config.Meth.GetSacid.radius,
            ...
            { options = {
                { event = "bayview_drugs:meth:progress", icon = "fas fa-fire", label = locale("MethRefuel"), menu = "MethGet" }
            },
                distance = 2.5
            }
        )
