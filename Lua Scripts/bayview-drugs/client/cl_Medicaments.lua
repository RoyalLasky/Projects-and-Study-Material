...
RegisterNetEvent("bayview_drugs:medicaments:menus", function(data)
        ...
        if Config.Context == "qbcore" then
            ...
            for _, v in pairs(Config.MedicamentsShop.Items) do
                options[#options+1] = {
                    ...
                    params = {
                        event = 'bayview_drugs:medicaments:progress',
...
RegisterNetEvent("bayview_drugs:medicaments:progress")
AddEventHandler("bayview_drugs:medicaments:progress", function(data)
    ...
    if amount then
        if tonumber(amount) >= data.min and tonumber(amount) <= data.max then
            lib.callback('bayview_drugs:medicaments:getitem', false, function(value)
                if value then
                    if not IsProgressBarActive() then
                        ...
                        TSE("bayview_drugs:medicaments:giveitems", data.item, data.price, amount)
                    end
                else
                ...
                if distance < 20 and not Spawned then
                    ...
                    if Config.InteractionType ~= "target" then
                        lib.zones.sphere({
                            ...
                            inside = function(self)
                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent("bayview_drugs:medicaments:menus", {shop = "medicaments"})
                                end
                        ...
                        target:AddTargetEntity(MedicamentNPC, {
                            options = {
                                {
                                    event = "bayview_drugs:medicaments:menus",
                                    icon = "fas fa-skull",
                                    label = locale("shop"),
                                    shop = "medicaments"
                                },
