        ...
        if Config.Context == "qbcore" then
            ...
            for k, v in pairs(Config.Drugs) do
                options[#options+1] = {
                    ...
                    params = {
                        event = 'bayview_drugs:SellChoose',
...
AddEventHandler('bayview_drugs:SellChoose', function(data)
    lib.callback('bayview_drugs:getpolice', false, function(value)
...
AddEventHandler('bayview_drugs:sell', function(data)
    if data.option == "sell" then
        lib.callback('bayview_drugs:selling:getitem', false, function(value)
            if value and Selling then
                ...
                TSE('bayview_drugs:selling:pay', math.ceil(price), sellingDrug, amount, label, math.ceil(oldprice))
                Client = nil
                if Config.InteractionType ~= "target" then
                    clientzone:remove()
                    clientzone = nil
