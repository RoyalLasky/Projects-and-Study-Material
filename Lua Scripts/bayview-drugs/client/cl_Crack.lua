...
SetTimeout(2000, function()
    if Config.InteractionType == "target" then
        ...
        target:AddCircleZone("CrackProcess", Config.Crack.Process.coords, Config.Crack.Process.radius,
            ...
            { options = {
                { event = "bayview_drugs:crack:menu", icon = "fas fa-pills", label = locale("MakeCrack"),
                    menu = "CrackProcess",
                }
            },
                distance = 2.5
        ...
        lib.zones.sphere({
            ...
            inside = function(self)
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("bayview_drugs:crack:menu", {menu = "CrackProcess"})
                end
...
RegisterNetEvent("bayview_drugs:crack:menu", function(data)
        ...
        if Config.Context == "qbcore" then
            exports['qb-menu']:openMenu({
                ...
                },
                {
                    ...
                    params = {
                        event = 'bayview_drugs:crack:progress',
                        args = { menu = data.menu }
                    }
                }
            })
...
RegisterNetEvent("bayview_drugs:crack:progress")
AddEventHandler("bayview_drugs:crack:progress", function(data)
    if data.menu == "CrackProcess" then
        lib.callback('bayview_drugs:crack:getitem', false, function(value)
            if value then
                if not IsProgressBarActive() then
                    ...
                    DeleteEntity(prop)
                    TSE("bayview_drugs:crack:giveitems", data.menu)
                end
            else
