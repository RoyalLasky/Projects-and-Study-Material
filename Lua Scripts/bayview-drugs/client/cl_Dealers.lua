    ...
    if Config.InteractionType == "target" then
        ...
        if Config.LocateDealer.enabled then
            ...
            target:AddCircleZone("DealerLocation1", Config.LocateDealer.Location.Coords,
                ...
                { options = {
                    { event = "bayview_drugs:dealers:progress", icon = "fas fa-desktop", label = locale("LocateDealer"),
                        shop = "LocateDealer1",
                    }
                },
                    distance = 2.5
        ...
        if Config.Gerald.enabled then
            ...
            target:AddCircleZone("GeraldLocation", Config.Gerald.Location.Coords, Config.Madrazo.Location.radius,
                ...
                { options = {
                    { event = "bayview_drugs:dealers:menus", icon = "fas fa-cannabis", label = locale("Gerald"),
                        shop = "GeraldShop",
                    }
                },
                    distance = 2.5
        ...
        if Config.Madrazo.enabled then
            ...
            target:AddCircleZone("MadrazoLocation", Config.Madrazo.Location.Coords, Config.Madrazo.Location.radius,
                ...
                { options = {
                    { event = "bayview_drugs:dealers:menus", icon = "fas fa-capsules", label = locale("Madrazo"),
                        shop = "MadrazoTrade",
                    }
                },
                    distance = 2.5
            ...
            lib.zones.sphere({
                ...
                inside = function(self)
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent("bayview_drugs:dealers:progress", {shop = "LocateDealer1"})
                    end
        ...
        if Config.Gerald.enabled then
            lib.zones.sphere({
                ...
                inside = function(self)
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent("bayview_drugs:dealers:menus", {shop = "GeraldShop"})
                    end
        ...
        if Config.Madrazo.enabled then
            lib.zones.sphere({
                ...
                inside = function(self)
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent("bayview_drugs:dealers:menus", {shop = "MadrazoTrade"})
                    end
...
RegisterNetEvent("bayview_drugs:dealers:menus")
AddEventHandler("bayview_drugs:dealers:menus", function(data)
                    ...
                    if Config.Context == "qbcore" then
                        exports['qb-menu']:openMenu({
                            ...
                            },
                            {
                                ...
                                params = {
                                    event = 'bayview_drugs:dealers:progress',
                                    args = { shop = data.shop }
                                }
                            }
                        })
...
RegisterNetEvent("bayview_drugs:dealers:progress")
AddEventHandler("bayview_drugs:dealers:progress", function(data)
    if data.shop == "DealerShop" then
        ...
        if amount then
            if tonumber(amount) >= data.min and tonumber(amount) <= data.max then
                lib.callback('bayview_drugs:dealersshop:getitem', false, function(value)
                    if value then
                        if not IsProgressBarActive() then
                            ...
                            TSE("bayview_drugs:dealersshop:giveitems", data.item, data.price, amount)
                        end
                    else
    ...
    elseif data.shop == "GeraldShop" then
        lib.callback('bayview_drugs:dealers:getitem', false, function(value)
            if value then
                if not IsProgressBarActive() then
                    GetSkin()
                    ...
                    ApplySkin()
                    SetEntityCollision(cache.ped, true, true)
                    TSE("bayview_drugs:dealers:giveitems", data.shop)
                end
            else
                Notify("error", locale("error"), locale("RequiredItems"))
            end
    ...
    elseif data.shop == "LocateDealer1" then
        lib.callback('bayview_drugs:dealers:getitem', fasle, function(value)
            if value then
                if not IsProgressBarActive() then
                ...
                if distance < 20 and not SpawnedDealer then
                    ...
                    if Config.InteractionType ~= "target" then
                        lib.zones.sphere({
                            ...
                            inside = function(self)
                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent("bayview_drugs:dealers:menus", {shop = "DealerShop"})
                                end
                        ...
                        target:AddTargetEntity(Dealer, {
                            options = {
                                {
                                    event = "bayview_drugs:dealers:menus",
                                    icon = "fas fa-store",
                                    label = locale("shop"),
                                    shop = "DealerShop"
                                },
