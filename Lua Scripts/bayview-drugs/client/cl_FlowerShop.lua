...
RegisterNetEvent("bayview_drugs:flowershop:menus", function(data)
        ...
        if Config.Context == "qbcore" then
            ...
            for _, v in pairs(Config.FlowerShop.Items) do
                options[#options+1] = {
                    ...
                    params = {
                        event = 'bayview_drugs:flowershop:progress',
...
RegisterNetEvent("bayview_drugs:flowershop:progress")
AddEventHandler("bayview_drugs:flowershop:progress", function(data)
    if data.shop == "flowershop" then
        ...
        if amount then
            if tonumber(amount) >= data.min and tonumber(amount) <= data.max then
                lib.callback('bayview_drugs:flowershop:getitem', false, function(value)
                    if value then
                        if not IsProgressBarActive() then
                            ...
                            TSE("bayview_drugs:flowershop:giveitems", data.item, data.price, amount)
                        end
                    else
                ...
                if distance < 20 and not Spawned then
                    ...
                    if Config.InteractionType ~= "target" then
                        lib.zones.sphere({
                            ...
                            inside = function(self)
                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent("bayview_drugs:flowershop:menus", {shop = "flowershop"})
                                end
                        ...
                        target:AddTargetEntity(FlowerNPC, {
                            options = {
                                {
                                    event = "bayview_drugs:flowershop:menus",
                                    icon = "fas fa-store",
                                    label = locale("shop"),
                                    shop = "flowershop"
                                },
