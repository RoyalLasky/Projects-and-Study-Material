...
ApplySkin = function()
    ...
    elseif Config.Clothing == "qb-clothing" then
        TriggerServerEvent("bayview_drugs:loadPlayerSkin")
...
RegisterNetEvent("bayview_drugs:loadSkin")
AddEventHandler("bayview_drugs:loadSkin", function(_, model, data)
...
RegisterNetEvent('bayview_drugs:consumables', function(text, animation, duration, effect, add)
...
function Status(add)
    if add.enabled then
        ...
        if add.stress.enabled then
            TriggerServerEvent('hud:server:RelieveStress', add.stress.remove)
        end
    end
end
...
RegisterNetEvent("bayview_drugs:consumables:menu")
AddEventHandler("bayview_drugs:consumables:menu", function(data)
        ...
        lib.registerContext({
            ...
            options = {
                [locale("BluntRoll")] = {
                    ...
                    event = 'bayview_drugs:consumables:progress',
                    args = { type = data }
                }
            }
        })
...
RegisterNetEvent("bayview_drugs:consumables:progress")
AddEventHandler("bayview_drugs:consumables:progress", function(data)
    if data == "xanax_pack" then
        if not IsProgressBarActive() then
            ...
            TSE("bayview_drugs:consumables:giveitems", data)
        end
    ...
    if data.type == "weed_wrap" then
        lib.callback('bayview_drugs:consumables:getitem', false, function(value)
            if value then
                if not IsProgressBarActive() then
                    ...
                    TSE("bayview_drugs:consumables:giveitems", data.type)
                end
            else
        ...
        lib.callback('bayview_drugs:consumables:getitem', false, function(value)
            if value then
                if not IsProgressBarActive() then
                    ...
                    TSE("bayview_drugs:consumables:giveitems", data.drug)
                end
            else
                Notify("error", locale("error"), locale("RequiredItems"))
            end
