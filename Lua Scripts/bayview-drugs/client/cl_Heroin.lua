...
RegisterNetEvent("bayview_drugs:heroin:menu", function(data)
		...
		if Config.Context == "qbcore" then
            exports['qb-menu']:openMenu({
                ...
                },
                {
                    ...
                    params = {
                        event = 'bayview_drugs:heroin:progress',
                        args = { menu = data.menu }
                    }
                }
            })
...
RegisterNetEvent("bayview_drugs:heroin:progress")
AddEventHandler("bayview_drugs:heroin:progress", function(data)
	if data.menu == "HeroinProcess" then
		lib.callback('bayview_drugs:heroin:getitem', false, function(value)
			if value then
				if not IsProgressBarActive() then
					...
					TSE("bayview_drugs:heroin:giveitems", data.menu)
				end
			else
...
SetTimeout(2000, function()
	if Config.InteractionType == "target" then
		...
		target:AddCircleZone("HeroinProcess", Config.Heroin.Process.coords, Config.Heroin.Process.radius,
			...
			{ options = {
				{ event = "bayview_drugs:heroin:menu", icon = "fas fa-prescription-bottle", label = locale("HeroinProcess"),
					menu = "HeroinProcess",
				}
			},
				distance = 2.5
		...
		lib.zones.sphere({
			...
			inside = function(self)
				if IsControlJustReleased(0, 38) then
					TriggerEvent("bayview_drugs:heroin:menu", {menu = "HeroinProcess"})
				end
		...
		lib.callback('bayview_drugs:heroin:getitem', false, function(value)
			if value then
				if not IsProgressBarActive() then
					...
					SpawnedHeroinPlants = SpawnedHeroinPlants - 1
					TSE('bayview_drugs:heroin:giveitems', "heroinPickup")
				end
			else
