...
RegisterNetEvent("bayview_drugs:pharmacist:Menus")
AddEventHandler("bayview_drugs:pharmacist:Menus", function(data)
                ...
                if Config.Context == "qbcore" then
                    ...
                    for _, v in pairs(Config.Pharmacist.Items) do
                        options[#options+1] = {
                            ...
                            params = {
                                event = 'bayview_drugs:pharmacist:progress',
...
RegisterNetEvent("bayview_drugs:pharmacist:progress")
AddEventHandler("bayview_drugs:pharmacist:progress", function(data)
    if data.shop == "PharmacistShop" then
        ...
        if amount then
            if tonumber(amount) >= data.min and tonumber(amount) <= data.max then
                lib.callback('bayview_drugs:pharmacist:getitem', false, function(value)
                    if value then
                        if not lib.progressActive() then
                            ...
                            TSE("bayview_drugs:pharmacist:giveitems", data.item, data.price, amount)
                        end
                    else
                ...
                if distance < 20 and not Spawned then
                    ...
                    if Config.InteractionType ~= "target" then
                        lib.zones.sphere({
                            ...
                            inside = function(self)
                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent("bayview_drugs:pharmacist:Menus", {shop = "PharmacistShop"})
                                end
                        ...
                        target:AddTargetEntity(Pharmacist, {
                            options = {
                                {
                                    event = "bayview_drugs:pharmacist:Menus",
