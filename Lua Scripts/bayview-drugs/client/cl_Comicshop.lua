...
RegisterNetEvent("bayview_drugs:comicshop:Menus", function(data)
        ...
        if Config.Context == "qbcore" then
            ...
            for _, v in pairs(Config.ComicShop.Items) do
                options[#options+1] = {
                    ...
                    params = {
                        event = 'bayview_drugs:comicshop:progress',
...
RegisterNetEvent("bayview_drugs:comicshop:progress")
AddEventHandler("bayview_drugs:comicshop:progress", function(data)
    if data.shop == "comicshop" then
        ...
        if amount then
            if tonumber(amount) >= data.min and tonumber(amount) <= data.max then
                lib.callback('bayview_drugs:comicshop:getitem', false, function(value)
                    if value then
                        if not IsProgressBarActive() then
                            ...
                            TSE("bayview_drugs:comicshop:giveitems", data.item, data.price, amount)
                        end
                    else
                ...
                if distance < 20 and not Spawned then
                    ...
                    if Config.InteractionType ~= "target" then
                        lib.zones.sphere({
                            ...
                            inside = function(self)
                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent("bayview_drugs:comicshop:Menus", {shop = "comicshop"})
                                end
                        ...
                        target:AddTargetEntity(ComicNPC, {
                            options = {
                                {
                                    event = "bayview_drugs:comicshop:Menus",
                                    icon = "fas fa-store",
                                    label = locale("shop"),
                                    shop = "comicshop"
                                },
