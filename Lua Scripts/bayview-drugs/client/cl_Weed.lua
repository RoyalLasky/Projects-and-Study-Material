    ...
    onEnter = function()
        ...
        if Config.InteractionType == "target" then
            ...
            target:AddCircleZone("WeedElectricMenu", Config.Weed.AC.coords, Config.Weed.AC.radius,
                ...
                { options = {
                    { event = "bayview_drugs:weed:progress", icon = "fas fa-wind", label = locale("turnonact"),
                        menu = "ElectricON", },
                    { event = "bayview_drugs:weed:progress", icon = "fas fa-wind", label = locale("turnoffact"),
                        menu = "ElectricOFF", }
                },
                    distance = 2.5
                })
            ...
            --    { options = {
            --        { event = "bayview_drugs:weed:progress", icon = "fas fa-door-open", label = locale("leave"),
            --            menu = "WeedDoor2",
            --        }
            --    },
            --        distance = 2.5
            ...
            target:AddCircleZone("WeedCleanMenu", Config.Weed.Clean.coords, Config.Weed.Clean.radius,
                ...
                { options = {
                    { event = "bayview_drugs:weed:menus", icon = "fas fa-cannabis", label = locale("WeedClean"),
                        menu = "WeedClean",
                        canInteract = function()
                            if Electricity then
                                return true
            ...
            target:AddCircleZone("WeedPackMenu", Config.Weed.Package.coords, Config.Weed.Package.radius,
                ...
                { options = {
                    { event = "bayview_drugs:weed:menus", icon = "fas fa-box-open", label = locale("WeedPack"),
                        menu = "WeedPack",
                        canInteract = function()
                            if Electricity then
                                return true
            ...
            weedlabzones[#weedlabzones+1] = lib.zones.sphere({
                ...
                inside = function(self)
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent("bayview_drugs:weed:progress", Electricity and {menu = "ElectricOFF"} or {menu = "ElectricON"})
                    end
            ...
            weedlabzones[#weedlabzones+1] = lib.zones.sphere({
                ...
                inside = function(self)
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent("bayview_drugs:weed:progress", {menu = "WeedDoor2"})
                    end
            ...
            weedlabzones[#weedlabzones+1] = lib.zones.sphere({
                ...
                inside = function(self)
                    if Electricity then
                        if IsControlJustReleased(0, 38) then
                            TriggerEvent("bayview_drugs:weed:menus", {menu = "WeedClean"})
                        end
            ...
            weedlabzones[#weedlabzones+1] = lib.zones.sphere({
                ...
                inside = function(self)
                    if Electricity then
                        if IsControlJustReleased(0, 38) then
                            TriggerEvent("bayview_drugs:weed:menus", {menu = "WeedPack"})
                        end
...
RegisterNetEvent("bayview_drugs:weed:menus", function(data)
        ...
        if Config.Context == "qbcore" then
            exports['qb-menu']:openMenu({
                ...
                },
                {
                    ...
                    params = {
                        event = 'bayview_drugs:weed:progress',
                        args = { menu = data.menu }
                    }
                }
            })
...
-- PROGRESSY --
RegisterNetEvent("bayview_drugs:weed:progress")
AddEventHandler("bayview_drugs:weed:progress", function(data)
    ...
    --    end
    if data.menu == "WeedClean" then
        lib.callback('bayview_drugs:weed:getitem', false, function(value)
            if value then
                if not IsProgressBarActive() then
                    ...
                    ProgressBar(GetAnimDuration(dict, "sorter_left_sort_v1_sorter01") * 1100, locale("WeedCleaning"))
                    ...
                    TSE("bayview_drugs:weed:giveitems", data.menu)
                ...
                lib.callback('bayview_drugs:weed:getitem', false, function(value)
                    if value == 1 then
                        ...
                        if value then
                            ...
                            while (not HasAnimDictLoaded(dict)) do Wait(0) end
                            TaskPlayAnim(cache.ped, dict, clip, 3.0, 1.0, -1, 49, 0, false, false, false)
                            local hash2 = `bkr_prop_weed_spray_01a`
                            RequestModel(hash2)
                            while not HasModelLoaded(hash2) do
                                Wait(100)
                                RequestModel(hash2)
                            ...
                            local prop2 = CreateObject(hash2, GetEntityCoords(cache.ped), true, true, true)
                            AttachEntityToEntity(prop2, cache.ped, GetPedBoneIndex(cache.ped, 57005), 0.1, -0.06, -0.03, 270.0, 0.0, 0.0, true, true, false, false, 1, true)
                            ProgressBar(7500, "Spraying Fertilizer")
                            DetachEntity(prop2, false, false)
                            DeleteEntity(prop2)
                            StopAnimTask(cache.ped, dict, clip, 1.0)
                            Wait(0)
                            ...
                            TSE("bayview_drugs:weed:giveitems", menu, entity)
                        else
                            Notify("error", locale("error"), locale("RequiredItems"))
                        end
                    end
    ...
    if Config.Weed.Enterlab.NeedItem then
        if Config.InteractionType == "target" then
            ...
            target:AddCircleZone("WeedTeleportMenu1", Config.Weed.Enterlab.coords, Config.Weed.Enterlab.radius,
                ...
                { options = {
                    { event = "bayview_drugs:weed:progress", icon = "fas fa-door-open", label = locale("enter"),
                        menu = "WeedDoor1",
            ...
            lib.zones.sphere({
                ...
                inside = function(self)
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent("bayview_drugs:weed:progress", {menu = "WeedDoor1", Card = Config.Weed.Enterlab.ItemName})
                    end
        ...
        if Config.InteractionType == "target" then
            ...
            lib.zones.sphere({
                ...
                inside = function(self)
                    if IsControlJustReleased(0, 38) then
                        TriggerEvent("bayview_drugs:weed:progress", {menu = "WeedDoor1"})
                    end
